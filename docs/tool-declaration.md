...
| `@RequestParam` | Query parameter; `required` unless it has a default value. On **write** methods it is also added to the tool's `queryParams` (KeelBase sends it as a query string instead of body). |
